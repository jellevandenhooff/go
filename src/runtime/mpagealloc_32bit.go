...
//go:build 386 || arm || mips || mipsle || wasm || wasm32 || (gccgo && (ppc || s390))

// wasm and wasm32 are treated as 32-bit architectures for the purposes of the
// page allocator, even though wasm has 64-bit pointers. This is because any
// wasm pointer always has its top 32 bits as zero, so the effective heap
// address space is only 2^32 bytes in size (see heapAddrBits).
