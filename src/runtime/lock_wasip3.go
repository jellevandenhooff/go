...
func notesleep(n *note) {
	throw("notesleep not supported by wasip3")
}

func notetsleep(n *note, ns int64) bool {
	throw("notetsleep not supported by wasip3")
	return false
}
...
// beforeIdle is called by findRunnable when no goroutines are runnable.
// On wasip3, it ensures the waitable set and netpoll subsystem are initialized.
// The actual blocking happens in netpoll(delay) via pause().
